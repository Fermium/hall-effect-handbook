...
3. Place the sample in the middle of the gap. Choose a proper value for the current $I_b$ within a suggested a suggested 25mA value. Optionally, adjust the gains for $V_H$, $V_R$ or $I_B$. Note : the resistance at higher temperature may exceed the value at room temperature by a factor 2, and also the $V_H$ signal increase with temperature.
...
8. Power Up the instrument on NNG Logger and create a new workspace.
9. Open temperature vs time graph on NNG Logger. 
10. Start data acquisition on NNG Logger
11. When the plot temperature vs time shows a slope close to zero, pause the data acquisition.
12. Empty the dewar (e.g. transferring the residual liquid nitrogen into another dewar), reposition the sample in the middle of the magnets-gap and start a new data acquisition for increasing temperature. 
13. When the temperature vs. time slope start approaching zero, switch ON the heater to full power.
14. Export the data to .csv
