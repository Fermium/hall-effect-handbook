...
fit <- lm( nisttable.k.interval$X2~poly(nisttable.k.interval$X1,5,raw = TRUE) )
...
A digitally controlled resistive element (heater) is wound around the base of the sample, allowing to heath it up after reaching room temperature. Ltk-hall-ge automatically shuts down the heater if $t \ge 150\, \,^{\circ}\mathrm{C}$.

The heater power can be set in percentage from the computer. Ltk-hall-ge will power it on-and-off (PWM - Pulse Width Modulation) with a variable duty cycle and a very long period of a few seconds. No measure is taken by the embedded datalogger during the transition period.

Warning: When using an external datalogger measures could be taken during the high-current spikes when the heater is automatically powered on or off. This will result in outliar measures that must be eliminated during the statistical analysis. This is not necessary if using the embedded datalogger.
