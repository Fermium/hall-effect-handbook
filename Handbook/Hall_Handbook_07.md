...
This behavior must be tested before proceeding to further measurements: if reversing the $B$ direction (i.e. rotating the sample of $180\,^{\circ}$ degrees) different values are measured, the offsets should be minimized using the potentiometer P in figure {@fig:sample_circuitry}.
...
A calibration of the magnetic field $B$ as a function of the gap $d$ may be made using a gauss-meter probe placed between the poles (see fig. {@fig:BvsGapD}). The magnetic field B should be measured using a gauss-meter probe placed at the center between the poles.
