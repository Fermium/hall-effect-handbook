...
The apparatus uses a Ge sample, cut from a single-crystal P-doped ingot, placed inside a isothermal
aluminum case.

The sample may be positioned into the gap between the opposite poles of two Nd-Fe-B permanent magnets. The
magnetic flux is confined by an “U shaped” soft-steel structure (see figure 5).
...
The PGA gains are selectable among the following values $G_{ PGA}= \{ 1,2,5,10,20,50,100,200\}$ through the front panel as shown in figure {@fig:frontPanel}. 
...
The display in the front panel shows the sample temperature in Celsius (calculated from the
measured thermocouple signal), the measured bias current $I_b$ (mA), the sample resistance $R$
(calculated from the measured voltage drop across the sample) and the selected values of the $V_H$
and $V_R$ channels. The displayed gain values are the product of DA and PGA gains for each
channels.
...
### Changing and measuring the magnetic field intensity
...
![In any type of thermocouple the sensitivity (Seebeck coefficient) does strongly depend on temperature . ](Media/seebeck_coefficient_vs_temperature.png){#fig:seebeckNonlinearity}
\ 

In order to get a correct measurement it is necessary to compensate for the non-linearity (see figure {@fig:seebeckNonlinearity}) of the thermocouple , e.g. using a fitting polynomial:

$$t=d_{ 0 }+d_{ 1 }E+d_{ 2 }E^{ 2 }+...+d_{ n }E^{ n }$$ {#eq:compensatingPolynomial}
where $E$ is the output voltage of the thermocouple in $mV$.

A fitting polynomial ({@eq:compensatingPolynomial}) of the fifth order is sufficient in our case , given the limited accuracy of our equipment.
...
The sample shown in {@fig:sample_pcb} (and used to obtain the data  in the following examples)  has thickness $t=0.5mm$, width $w=10mm$ and lenght $l=15mm$.
...
Figure {@fig:Output_voltages_versus_time} shows the measured values of the 3 output signals vs temperature obtained with a constant bias current $I_B=10mA$ and in a 0.4 $T$ magnetic field, using Vernier-LabPro interface [^Vernier]. The plot shows *Potential 1*  $= V_{out \, T}$, *Potential 2* $= V_{out \,H}$, *Potential 3* $= V_{out \, R}$. 

 [^Vernier]:[www.vernier.com](http://www.vernier.com)
