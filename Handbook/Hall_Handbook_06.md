...
The apparatus uses a Ge sample, cut from a single-crystal P-doped ingot, placed inside an aluminum case. 

The sample may be positioned into the gap between the opposite poles of two Ni-Fe-B permanent magnet discs. The magnetic flux is confined by an  "U" shaped soft-steel core (see Figure[FIX] ).
...
![Printed circuit board with germanium sample and thermocouple](Media/sample_pcb.jpg){#fig:sample_pcb}
\ 
...
[^GainMayChange]: The gain can change due to specifications and calibration. Please refer to the calculated values displayed on the front panel.
...
The output voltages on the front panel (BNC ports) are restrained in a number of cases:
